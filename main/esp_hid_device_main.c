...
static const char *index_html =
    ...
    "return;\n"
    "            let c = '';\n"
    "            if(e.key.length === 1) c = e.key;\n"
    "            else if(e.key === \"Enter\") c = \"\\n\";\n"
    "            else if(e.key === \"Backspace\") c = String.fromCharCode(8);\n"
    "            else if(e.key === \"Tab\") c = \"\\t\";\n"
    "            else if(e.key === \"Escape\") c = String.fromCharCode(27);\n"
    "            if(c !== '') sd('K', m + '|' + c);\n"
    ...
    "document.activeElement.tagName !== 'INPUT') kb.focus();\n"
    "        });\n"
    "        document.getElementById('kb').addEventListener('keydown', e => {\n"
    "            if (e.code === 'AltRight') { e.preventDefault(); sd('T', 64); return; }\n"
    "            if (e.code === 'ControlRight') { e.preventDefault(); sd('T', 16); return; }\n"
    "            const map = { 'ArrowUp':82, 'ArrowDown':81, 'ArrowLeft':80, 'ArrowRight':79, \n"
    "                          'Backspace':42, 'Enter':40, 'Tab':43, 'Delete':76, 'Escape':41 };\n"
    "            if (map[e.key]) {\n"
    "                e.preventDefault();\n"
    "                let m = (e.ctrlKey?1:0)|(e.shiftKey?2:0)|(e.altKey?4:0)|(e.metaKey?8:0);\n"
    "                sd('R', m + '|' + map[e.key]);\n"
    "            }\n"
    "        });\n"
    "    </script>\n"
