...
// static const char * gap_bt_prop_type_names[5] =
// {"","BDNAME","COD","RSSI","EIR"};
...
static const char *bt_gap_evt_names[] = {"DISC_RES",  "DISC_STATE_CHANGED",
                                         "RMT_SRVCS", "RMT_SRVC_REC",
                                         "AUTH_CMPL", "PIN_REQ",
                                         "CFM_REQ",   "KEY_NOTIF",
                                         "KEY_REQ",   "READ_RSSI_DELTA"};
...
static void add_bt_scan_result(esp_bd_addr_t bda, esp_bt_cod_t *cod,
                               esp_bt_uuid_t *uuid, uint8_t *name,
...
static void add_ble_scan_result(esp_bd_addr_t bda,
                                ...
                                uint16_t appearance, uint8_t *name,
                                uint8_t name_len, int rssi) {
  ...
  esp_hid_scan_result_t *r =
      (esp_hid_scan_result_t *)malloc(sizeof(esp_hid_scan_result_t));
  if (r == NULL) {
...
void print_uuid(esp_bt_uuid_t *uuid) {
  if (uuid->len == ESP_UUID_LEN_16) {
    ...
    GAP_DBG_PRINTF(
        ...
        "%02x,%02x",
        uuid->uuid.uuid128[0], uuid->uuid.uuid128[1], uuid->uuid.uuid128[2],
        uuid->uuid.uuid128[3], uuid->uuid.uuid128[4], uuid->uuid.uuid128[5],
        uuid->uuid.uuid128[6], uuid->uuid.uuid128[7], uuid->uuid.uuid128[8],
        uuid->uuid.uuid128[9], uuid->uuid.uuid128[10], uuid->uuid.uuid128[11],
        uuid->uuid.uuid128[12], uuid->uuid.uuid128[13], uuid->uuid.uuid128[14],
...
static void handle_bt_device_result(struct disc_res_param *disc_res) {
  ...
  for (int i = 0; i < disc_res->num_prop; i++) {
    ...
    if (prop->type == ESP_BT_GAP_DEV_PROP_BDNAME) {
      ...
      GAP_DBG_PRINTF("major: %s, minor: %d, service: 0x%03x",
                     esp_hid_cod_major_str(cod->major), cod->minor,
                     cod->service);
      ...
      data = esp_bt_gap_resolve_eir_data(
          (uint8_t *)prop->val, ESP_BT_EIR_TYPE_CMPL_16BITS_UUID, &len);
      if (data == NULL) {
        data = esp_bt_gap_resolve_eir_data(
            (uint8_t *)prop->val, ESP_BT_EIR_TYPE_INCMPL_16BITS_UUID, &len);
      }
      ...
      data = esp_bt_gap_resolve_eir_data(
          (uint8_t *)prop->val, ESP_BT_EIR_TYPE_CMPL_32BITS_UUID, &len);
      if (data == NULL) {
        data = esp_bt_gap_resolve_eir_data(
            (uint8_t *)prop->val, ESP_BT_EIR_TYPE_INCMPL_32BITS_UUID, &len);
      }
      ...
      data = esp_bt_gap_resolve_eir_data(
          (uint8_t *)prop->val, ESP_BT_EIR_TYPE_CMPL_128BITS_UUID, &len);
      if (data == NULL) {
        data = esp_bt_gap_resolve_eir_data(
            (uint8_t *)prop->val, ESP_BT_EIR_TYPE_INCMPL_128BITS_UUID, &len);
      }
...
#if CONFIG_BT_BLE_ENABLED
static void
handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst) {
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event,
  ...
  case ESP_BT_GAP_PIN_REQ_EVT: {
    ESP_LOGI(TAG, "BT GAP PIN_REQ_EVT min_16_digit:%d",
  ...
  case ESP_BT_GAP_CFM_REQ_EVT:
    ESP_LOGI(TAG,
             "BT GAP CFM_REQ_EVT Please compare the numeric value: %06" PRIu32,
             param->cfm_req.num_val);
    esp_bt_gap_ssp_confirm_reply(param->cfm_req.bda, true);
    break;
  case ESP_BT_GAP_KEY_NOTIF_EVT:
    ESP_LOGI(TAG, "BT GAP KEY_NOTIF_EVT passkey:%06" PRIu32,
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event,
  ...
  case ESP_GAP_BLE_SCAN_RESULT_EVT: {
    ...
    case ESP_GAP_SEARCH_INQ_CMPL_EVT:
      ESP_LOGV(TAG, "BLE GAP EVENT SCAN DONE: %d",
  ...
  case ESP_GAP_BLE_AUTH_CMPL_EVT:
    if (!param->ble_security.auth_cmpl.success) {
      ...
      ESP_LOGE(TAG, "BLE GAP AUTH ERROR: 0x%x",
  ...
  case ESP_GAP_BLE_KEY_EVT: // shows the ble key info share with peer device to
                            // the user.
    ESP_LOGI(TAG, "BLE GAP KEY type = %s",
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance,
  ...
  // esp_ble_io_cap_t iocap = ESP_IO_CAP_OUT;//you have to enter the key on the
  // host esp_ble_io_cap_t iocap = ESP_IO_CAP_IN;//you have to enter the key on
  // the device
  esp_ble_io_cap_t iocap =
      ESP_IO_CAP_IO; // you have to agree that key matches on both
  // esp_ble_io_cap_t iocap = ESP_IO_CAP_NONE;//device is not capable of input
  // or output, insecure
  ...
  if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_AUTHEN_REQ_MODE,
                                            &auth_req, 1)) != ESP_OK) {
    ESP_LOGE(TAG, "GAP set_security_param AUTHEN_REQ_MODE failed: %d", ret);
    return ret;
  }

  if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_IOCAP_MODE, &iocap,
                                            1)) != ESP_OK) {
    ESP_LOGE(TAG, "GAP set_security_param IOCAP_MODE failed: %d", ret);
    return ret;
  }

  if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_SET_INIT_KEY, &init_key,
                                            1)) != ESP_OK) {
    ESP_LOGE(TAG, "GAP set_security_param SET_INIT_KEY failed: %d", ret);
    return ret;
  }

  if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_SET_RSP_KEY, &rsp_key,
                                            1)) != ESP_OK) {
    ESP_LOGE(TAG, "GAP set_security_param SET_RSP_KEY failed: %d", ret);
    return ret;
  }

  if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_MAX_KEY_SIZE, &key_size,
                                            1)) != ESP_OK) {
    ESP_LOGE(TAG, "GAP set_security_param MAX_KEY_SIZE failed: %d", ret);
    return ret;
  }

  if ((ret = esp_ble_gap_set_security_param(ESP_BLE_SM_SET_STATIC_PASSKEY,
...
static int nimble_hid_gap_event(struct ble_gap_event *event, void *arg) {
  ...
  case BLE_GAP_EVENT_CONNECT:
    ...
    if (event->connect.status == 0) {
      /* Trigger security immediately to prompt Android to re-verify bond */
...
esp_err_t esp_hid_scan(uint32_t seconds, size_t *num_results,
                       esp_hid_scan_result_t **results) {
  if (num_bt_scan_results || bt_scan_results || num_ble_scan_results ||
      ble_scan_results) {
